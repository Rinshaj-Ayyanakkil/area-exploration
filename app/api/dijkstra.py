from app.utils.dbconnect import select_all
...
class pathFinder:
    def find_path(self, source_id, destination_id):
        ...
        query = "SELECT * FROM `wifi_settings` ORDER BY `wifi_id` DESC"
        ...
        for hotspot in hotspots:
            id = str(hotspot["wifi_id"])
            k = Node(id)
            hotspots_as_nodes.append(k)
            if id == source_id:
                source_node = k
            if id == destination_id:
                destination_node = k
        w_graph = Graph.create_from_nodes(hotspots_as_nodes)

        query = "SELECT * FROM `paths` ORDER BY `path_id` DESC"
        paths = select_all(query)
        if len(paths) > 0:
            for path in paths:
                left_edge = None
                right_edge = None
                for node in hotspots_as_nodes:
                    if str(node.data) == str(path["landmark_A"]):
                        left_edge = node
                    if str(node.data) == str(path["landmark_B"]):
                        right_edge = node
                    if left_edge != None and right_edge != None:
                        break

                w_graph.connect(left_edge, right_edge, 1)

        path = []
        for (weight, node) in w_graph.dijkstra(source_node):
            if str(node[-1].data) == destination_id:
                path = node
                break

        route_to_destination = []
        for n in path:
            route_to_destination.append(n.data)
